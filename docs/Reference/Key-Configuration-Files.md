...
!!! example
 ...
 | **knownServersFile** | File used by Web3Signer to trust the Interlock server certificate. This file is automatically generated if it does not already exist. The file format is, `host:port sha_signature_of_interlock_certificate` and uses the Trust On First Use (TOFU) mode. |
